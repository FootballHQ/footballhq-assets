(() => {
  ...
  function openRecords(){updateBestDisplay();els.recordsModal.classList.remove('hidden')}function closeRecords(){els.recordsModal.classList.add('hidden')}
  ...
  function calculateOfficialBase(c){const skill=c.speed*.58+c.burst*.27+c.consistency*.15;const base=5.24-((skill-50)*.0205)+(Math.random()-.5)*.12;return clamp(base,4.35,5.25)}
  ...
  function runInteractiveRace(token){return new Promise(resolve=>{
    ...
    const playerVisualSeconds=baseA*1.78;
    const cpuVisualSeconds=baseB*1.52;
    raceStartedAt=performance.now();raceLive=true;els.runnerA.classList.add('running');els.runnerB.classList.add('running');
    let last=raceStartedAt;
    function frame(now){

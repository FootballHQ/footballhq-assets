...
function load(i){if(i>=files.length){window.dispatchEvent(new CustomEvent('turf:static-runtime-ready'));return;}var s=document.createElement('script');s.src=base+files[i];s.async=false;s.onload=function(){load(i+1)};s.onerror=function(){console.warn('TURF static runtime patch failed',files[i]);load(i+1)};(document.body||document.documentElement).appendChild(s)}

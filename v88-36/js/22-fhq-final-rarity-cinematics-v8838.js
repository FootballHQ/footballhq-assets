/* FootballHQ V88.38 — FINAL RARITY CINEMATICS + CAROUSEL HOOK
...
(function(){
  'use strict';
  if (window.__FHQ_V8838_RARITY_RUNTIME__) return;
  window.__FHQ_V8838_RARITY_RUNTIME__ = true;

  const ORDER = ['COMMON','UNCOMMON','RARE','EPIC','LEGENDARY','OBSIDIAN','SIGNATURE'];
  const DURATION = {
    COMMON:1450, UNCOMMON:1750, RARE:1950, EPIC:2200,
    LEGENDARY:2600, OBSIDIAN:3300, SIGNATURE:3900
  };
  ...
  .fhq8837-cardslot{position:absolute;left:50%;top:53%;width:min(34vw,330px);aspect-ratio:720/1040;transform:translate(-50%,-50%) scale(.12);opacity:0;
    border-radius:22px;box-shadow:0 40px 120px rgba(0,0,0,.82);overflow:hidden;z-index:50;will-change:transform,opacity}
  .fhq8837-cardslot img{width:100%;height:100%;object-fit:contain;display:block}
  ...
  function buildBase(rarity, imgSrc){
    ...
    if(imgSrc){ const im=new Image(); im.src=imgSrc; card.appendChild(im); }
  ...
  function finish(ctx, duration){
    setTimeout(()=>{
      ctx.o.classList.remove('on');
      setTimeout(()=>ctx.o.remove(),220);
    },duration);
  }

  function revealCard(ctx, delay, rarity){
    setTimeout(()=>{
      ctx.flash.style.animation='fhq8837Flash .72s ease-out both';
      ctx.card.style.animation='fhq8837CardIn .95s cubic-bezier(.18,.8,.2,1) both';
      ctx.label.animate([{opacity:0,transform:'translateX(-50%) translateY(-8px)'},{opacity:1,transform:'translateX(-50%) translateY(0)'}],{duration:430,fill:'both'});
      setTimeout(()=>ctx.chrome.style.animation='fhq8837Chrome .8s ease both',650);
    },delay);
  }
  ...
  function play(rarity,imgSrc){
    ...
    const ctx=buildBase(rarity,imgSrc);
    ({COMMON:common,UNCOMMON:uncommon,RARE:rare,EPIC:epic,LEGENDARY:legendary,OBSIDIAN:obsidian,SIGNATURE:signature}[rarity])(ctx);
  }
  ...
  window.FHQ_playRarityCinematic = function(rarity, cardOrUrl){
    ...
    const img=typeof cardOrUrl==='string' ? cardOrUrl : extractImage(cardOrUrl);
    play(r,img);
  };
  ...
  function rarityFromCardId(src){
    const m=String(src||'').match(/tg0*([0-9]{1,3})/i);
    if(!m) return null;
    const n=parseInt(m[1],10);
    if(n>=1 && n<=5) return 'COMMON';
    if(n>=6 && n<=10) return 'UNCOMMON';
    if(n>=11 && n<=15) return 'RARE';
    if(n>=16 && n<=18) return 'EPIC';
    if(n===19) return 'LEGENDARY';
    if(n===20) return 'OBSIDIAN';
    if(n>=21 && n<=24) return 'SIGNATURE';
    return null;
  }
  ...
  function scoreImage(img){
    ...
    // Size dominates, then closeness to screen center.
    return (r.width*r.height) - dx*420 - dy*170;
  }

  function findCenterPackCard(){
    const overlay=document.getElementById('fhqPackOverlay');
    if(!overlay || !visible(overlay)) return null;

    const imgs=[...overlay.querySelectorAll('img')].filter(img=>{
      if(!visible(img)) return false;
      const src=String(img.currentSrc||img.src||'');
      return /tg0*\d+/i.test(src) || /gridiron/i.test(src);
    });
  ...
  function rarityNearImage(img){
    if(!img) return null;

    // Most reliable: card asset id/range.
    let r=rarityFromCardId(img.currentSrc||img.src);
    if(r) return r;

    // Fallback: inspect nearby rendered card text.
    let node=img;
    for(let i=0;i<6 && node;i++,node=node.parentElement){
      const text=String(node.textContent||'').toUpperCase();
      for(let j=ORDER.length-1;j>=0;j--){
        if(text.includes(ORDER[j])) return ORDER[j];
      }
    }
    return null;
  }
  ...
  function scanLiveCarousel(force){
    ...
    scanTimer=setTimeout(()=>{
      const img=findCenterPackCard();
      if(!img) return;

      const src=String(img.currentSrc||img.src||'');
      const rarity=rarityNearImage(img);
      if(!rarity || !src) return;

      const key=rarity+'|'+src;
      if(!force && key===lastCenterKey) return;
      lastCenterKey=key;

      // One cinematic per appearance of a center card. A later pack can show
      // the same card again after the overlay closes/reopens.
      const overlay=document.getElementById('fhqPackOverlay');
      ...
      // Give the carousel a fraction of a beat to settle, then cover it
      // with the rarity cinematic.
      setTimeout(()=>play(rarity,src),110);
    },70);
  }
  ...
  function bindCarouselHook(){
    ...
    if(overlay.dataset.fhq8838Bound==='1') return true;
    overlay.dataset.fhq8838Bound='1';
    overlay.dataset.fhq8838Session='0';
    ...
    const updateSession=()=>{
      ...
      if(nowOpen && !wasOpen){
        ...
        lastCenterKey='';
        setTimeout(()=>scanLiveCarousel(true),180);
      }
      wasOpen=nowOpen;
    };
    ...
    // Carousel arrows / NEXT interactions often only modify transforms/classes.
    overlay.addEventListener('click',()=>{
      setTimeout(()=>scanLiveCarousel(true),140);
  ...
  console.log('[FootballHQ] V88.38 rarity cinematics ready — Signature: Shockwave Crater; Obsidian: Royal Obsidian.');
})();

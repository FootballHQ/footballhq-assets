...
,
  ...
  /* ============================================================
     V88.36 FIX2 — RESTORE SPLIT-LOST FUNCTIONS INSIDE MAIN IIFE
     These MUST live here so they can reach private shop/collection
     renderers, fallbacks, and runtime state.
     ============================================================ */
  ...
  function openFootballHQSection(section){
      ...
      if(section==='shop'){sitePage='shop';document.body.classList.add('shop-page');if(core)core.style.display='none';if(shopPage){shopPage.classList.remove('hidden');shopPage.style.display='block'}fhqFix3LoadShop();window.scrollTo(0,0);return;}

      if(section==='locker'){sitePage='locker';document.body.classList.add('locker-page');if(core)core.style.display='none';if(lockerPage){lockerPage.classList.remove('hidden');lockerPage.style.display='block'}fhqFix3RenderLocker();window.scrollTo(0,0);return;}

      if(section==='album'){sitePage='album';document.body.classList.add('album-page');if(core)core.style.display='none';if(albumPage){albumPage.classList.remove('hidden');albumPage.style.display='block'}fhqFix3LoadCollections();window.scrollTo(0,0);return;}
  ...
  /* ============================================================
     V88.36 FIX3 — ROBUST SHOP / LOCKER / COLLECTION RENDERING
     These wrappers live INSIDE the main runtime IIFE so they can
     access the private fallback catalogs, previews, purchase/equip
     handlers, and account state.
     ============================================================ */

  function fhqFix3SafePreview(fn, fallbackHtml){
    try{
      const out=fn();
      return out||fallbackHtml;
    }catch(e){
      console.error('FootballHQ preview render error',e);
      return fallbackHtml;
    }
  }

  function fhqFix3RenderShopFallback(error){
    console.error('FootballHQ Shop primary renderer failed',error);
    ...
    const packs=(Array.isArray(FHQ_PACK_FALLBACK)?FHQ_PACK_FALLBACK:[]);
    const items=(Array.isArray(FHQ_SHOP_FALLBACK)?FHQ_SHOP_FALLBACK:[]);
    const cardPacks=packs.filter(function(pk){return pk&&pk.category==='card'});
    const avatarPack=packs.find(function(pk){return pk&&pk.category==='avatar'});

    function packHTML(pk){
      const preview=fhqFix3SafePreview(
        function(){return fhqPackPreview(pk)},
        '<div class="fhq-fix3-placeholder"><strong>'+esc(pk.name||'HQ Pack')+'</strong><span>'+esc(pk.guaranteeLabel||((pk.count||1)+' CARDS'))+'</span></div>'
      );
      return '<article class="fhq-shop-item fhq-pack-card '+esc(pk.rarity||'')+'" data-shop-pack-card="'+esc(pk.id)+'">'+
        '<div class="fhq-shop-preview">'+preview+'</div>'+
        '<h3>'+esc(pk.name||'HQ Pack')+'</h3>'+
        '<div class="fhq-v8831-pack-meta">'+esc(pk.guaranteeLabel||((pk.count||1)+' CARDS'))+'</div>'+
        '<p>'+esc(pk.description||'Football HQ card pack.')+'</p>'+
        '<div class="fhq-shop-buy"><span><span class="fhq-coin-icon" style="display:inline-grid;width:18px;height:18px;vertical-align:middle"></span> '+Number(pk.price||0)+
        '</span><button data-pack-buy="'+esc(pk.id)+'">PURCHASE</button></div></article>';
    }

    function avatarHTML(i){
      const owned=inv.some(function(x){return x&&x.source==='shop'&&x.shopId===i.id});
      const preview=fhqFix3SafePreview(
        function(){return fhqShopPreview(i)},
        '<div class="fhq-fix3-placeholder"><strong>'+esc(i.name||'Avatar')+'</strong></div>'
      );
      return '<article class="fhq-shop-item '+esc(i.rarity||'')+'">'+
        '<div class="fhq-shop-preview">'+preview+'</div>'+
        '<h3>'+esc(i.name||'Avatar')+'</h3><p>'+esc(i.description||'Football HQ avatar.')+'</p>'+
        '<div class="fhq-shop-buy"><span><span class="fhq-coin-icon" style="display:inline-grid;width:18px;height:18px;vertical-align:middle"></span> '+Number(i.price||0)+'</span>'+
        (owned?'<b class="fhq-shop-owned">OWNED</b>':'<button data-shop-buy="'+esc(i.id)+'">PURCHASE</button>')+
        '</div></article>';
    }

    let html='';
    if(filter==='pack'){
      html='<div class="fhq-v8831-shop-heading"><span>PACKS</span><h2>Card Packs</h2><p>Collection cards only.</p></div>'+cardPacks.map(packHTML).join('');
    }else if(filter==='avatar'){
      html='<div class="fhq-v8831-shop-heading"><span>AVATARS</span><h2>Avatars</h2><p>Profile cosmetics and avatar packs.</p></div>'+
        (avatarPack?packHTML(avatarPack):'')+items.filter(function(i){return i&&i.type==='avatar'}).map(avatarHTML).join('');
    }else{
      html='<div class="fhq-v8831-shop-heading"><span>FEATURED</span><h2>Featured Packs</h2><p>The Football HQ packs currently in rotation.</p></div>'+
        cardPacks.map(packHTML).join('');
    }

    if(!html.trim()){
      html='<div class="fhq-locker-empty"><strong>Shop is loading</strong><span>Please refresh once.</span></div>';
    }
    g.innerHTML=html;

    g.querySelectorAll('[data-shop-buy]').forEach(function(b){
      b.onclick=function(){try{fhqBuyShopItem(this.dataset.shopBuy)}catch(e){console.error(e)}};
    });
    g.querySelectorAll('[data-pack-buy]').forEach(function(b){
      b.onclick=function(){try{if(typeof fhqV8831UnlockAudio==='function')fhqV8831UnlockAudio();fhqBuyPack(this.dataset.packBuy)}catch(e){console.error(e)}};
    });
  }

  function fhqFix3LoadShop(){
    const g=document.getElementById('fhqShopGrid');
    if(g)g.innerHTML='';
    try{
      const local=Object.assign({},getAccountProfile(),{hqCoins:fhqCachedCoins()});
      fhqRenderShop({profile:local,items:FHQ_SHOP_FALLBACK,packs:FHQ_PACK_FALLBACK});
      if(!g||!String(g.innerHTML||'').trim()) throw new Error('Shop renderer returned empty HTML');
      if(fhqHasServer()){
        google.script.run
          .withSuccessHandler(function(x){
            try{fhqRenderShop(x);if(g&&!String(g.innerHTML||'').trim())fhqFix3RenderShopFallback(new Error('Server shop response rendered empty'))}
            catch(e){fhqFix3RenderShopFallback(e)}
          })
          .withFailureHandler(function(e){console.warn(e)})
          .getFootballHQShop(fhqGetToken());
      }
    }catch(e){
      fhqFix3RenderShopFallback(e);
    }
  }

  function fhqFix3RenderLockerFallback(error){
    ...
    if(!list.length){
      grid.innerHTML='<div class="fhq-locker-empty"><div class="empty-icon">◇</div><strong>No items in this filter yet</strong><span>Earn items through packs, collections, HQ Pass, and leaderboards.</span></div>';
      return;
    }

    grid.innerHTML=list.map(function(x){
      const preview=fhqFix3SafePreview(
        function(){return fhqOwnedPreview(x)},
        '<div class="fhq-fix3-placeholder"><strong>'+esc(x.name||x.value||'Owned Item')+'</strong></div>'
      );
      return '<article class="fhq-owned-item" data-item-type="'+esc(x.type||'item')+'" data-rarity="'+esc(x.rarity||'common')+'">'+
        '<div class="fhq-owned-preview">'+preview+'</div><strong>'+esc(x.name||x.value||'Owned Item')+'</strong>'+
        '<small>'+esc(x.source||'Owned')+'</small></article>';
    }).join('');
  }

  function fhqFix3RenderLocker(){
    const grid=document.getElementById('fhqLockerGrid');
    if(grid)grid.innerHTML='';
    try{
      fhqRenderLocker();
      if(!grid||!String(grid.innerHTML||'').trim())throw new Error('Locker renderer returned empty HTML');
    }catch(e){
      fhqFix3RenderLockerFallback(e);
    }
  }

  function fhqFix3CollectionsFallback(error){
    console.error('FootballHQ Collections primary renderer failed',error);
    const root=document.getElementById('fhqAlbumGrid');if(!root)return;
    const catalog=Array.isArray(FHQ_CARD_CATALOG_FALLBACK)?FHQ_CARD_CATALOG_FALLBACK:
      (Array.isArray(window.__fhqCardCatalog)?window.__fhqCardCatalog:[]);
    const owned=(window.__fhqCosmetics&&Array.isArray(window.__fhqCosmetics.collection))?window.__fhqCosmetics.collection:[];
    const sets={};
    catalog.forEach(function(c){if(!c)return;const s=c.set||'Football HQ';(sets[s]||(sets[s]=[])).push(c)});
    const names=Object.keys(sets);
    if(!names.length){
      root.innerHTML='<div class="fhq-locker-empty"><strong>Collections are loading</strong><span>Please refresh once.</span></div>';
      return;
    }
    root.innerHTML=names.map(function(name){
      const cards=sets[name],got=cards.filter(function(c){return owned.includes(c.id)}).length;
      let cover='';
      try{cover=fhqCollectionCoverArt(name,cards)}catch(e){}
      if(!cover){
        const first=cards[0]||{};
        cover='<div class="fhq-fix3-collection-cover"><strong>'+esc(name)+'</strong><span>'+cards.length+' CARDS</span></div>';
      }
      return '<article class="fhq-album-card" data-fix3-set="'+esc(name)+'">'+
        '<div class="fhq-album-cover">'+cover+'</div>'+
        '<div class="fhq-album-info"><strong>'+esc(name)+'</strong><span>'+got+' / '+cards.length+' owned</span></div></article>';
    }).join('');

    root.querySelectorAll('[data-fix3-set]').forEach(function(el){
      el.onclick=function(){
        try{
  ...
  function fhqFix3LoadCollections(){
    ...
    try{
      ...
      setTimeout(function(){
        try{
          if(root&&!String(root.innerHTML||'').trim())fhqFix3CollectionsFallback(new Error('Collections renderer returned empty HTML'));
        }catch(e){fhqFix3CollectionsFallback(e)}
      },0);
    }catch(e){
      fhqFix3CollectionsFallback(e);
    }
  }

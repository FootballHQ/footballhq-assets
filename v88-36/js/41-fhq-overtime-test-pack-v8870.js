/* ============================================================
   FOOTBALL HQ V88.70 — TEMP OVERTIME TEST PACK FRONTEND
   GitHub path: v88-36/js/41-fhq-overtime-test-pack-v8870.js
   PURPOSE: force a fresh frontend filename + unique runtime guard
   so the temporary Overtime Pack cannot be blocked by cached V88.49.
   ============================================================ */
(function(){
'use strict';
if(window.__FHQ_V8870_OVERTIME_SHOP__)return;
window.__FHQ_V8870_OVERTIME_SHOP__=true;
...
const ART={
  rookie_cards:A+'pack-rookie-v8848.png?v=8870',
  scrimmage_pack:A+'pack-scrimmage-v8848.png?v=8870',
  gametime_pack:A+'pack-gametime-v8848.png?v=8870',
  redzone_pack:A+'pack-redzone-v8848.png?v=8870',
  fourthquarter_pack:A+'pack-fourthquarter-v8848.png?v=8870',
  hailmary_pack:A+'pack-hailmary-v8848.png?v=8870',
  primetime_pack:A+'pack-primetime-v8848.png?v=8870',
  overtime_pack:A+'pack-primetime-v8848.png?v=8870'
};

function esc2(s){return String(s==null?'':s).replace(/[&<>\"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]})}
function packPreview8870(pk){
  const url=ART[pk.id];
  if(url)return '<div class="fhq-v8870-pack-art"><img src="'+url+'" alt="'+esc2(pk.name)+'" draggable="false"></div>';
  if(typeof fhqPackPreview==='function')try{return fhqPackPreview(pk)}catch(e){}
  return '<div class="fhq-v8870-pack-art fallback"><strong>'+esc2(pk.name)+'</strong></div>';
}
  ...
  const render=function(x){
    try{
      ...
      function packHTML(pk){
        return '<article class="fhq-shop-item fhq-pack-card '+esc2(pk.rarity||'')+'" data-shop-pack-card="'+esc2(pk.id)+'">'+
          '<div class="fhq-shop-preview">'+packPreview8870(pk)+'</div><h3>'+esc2(pk.name)+'</h3>'+ 
          '<div class="fhq-v8831-pack-meta">'+esc2(pk.guaranteeLabel||((pk.count||1)+' CARDS'))+'</div>'+ 
          '<p>'+esc2(pk.description||'Football HQ card pack.')+'</p><div class="fhq-shop-buy"><span><span class="fhq-coin-icon" style="display:inline-grid;width:18px;height:18px;vertical-align:middle"></span> '+pk.price+
          ' <button class="fhq-pack-odds-btn" data-pack-odds="'+esc2(pk.id)+'" aria-label="View pack odds">i</button></span><button data-pack-buy="'+esc2(pk.id)+'">PURCHASE</button></div></article>';
      }
      function avatarHTML(i){
        const owned=inv.some(v=>v&&v.source==='shop'&&v.shopId===i.id);
        if(typeof fhqShopPreview!=='function')return '';
        return '<article class="fhq-shop-item '+esc2(i.rarity||'')+'"><div class="fhq-shop-preview">'+fhqShopPreview(i)+'</div><h3>'+esc2(i.name)+'</h3><p>'+esc2(i.description||'Football HQ avatar.')+'</p><div class="fhq-shop-buy"><span><span class="fhq-coin-icon" style="display:inline-grid;width:18px;height:18px;vertical-align:middle"></span> '+i.price+'</span>'+(owned?'<b class="fhq-shop-owned">OWNED</b>':'<button data-shop-buy="'+esc2(i.id)+'">PURCHASE</button>')+'</div></article>';
      }
      let html='';
    ...
    }catch(e){console.warn('[FHQ V88.70] render failed',e);return oldRender(x)}
  };
  try{window.fhqRenderShop=render}catch(e){}
  try{fhqRenderShop=render}catch(e){}
  return true;
}

function injectStyle(){
  if(document.getElementById('fhqV8870ShopStyle'))return;
  const s=document.createElement('style');s.id='fhqV8870ShopStyle';s.textContent='\
  .fhq-v8870-pack-art{width:100%;height:100%;min-height:235px;display:grid;place-items:center;overflow:hidden;background:radial-gradient(circle at 50% 38%,#173040 0,#091218 60%,#05090c 100%);border-radius:14px}\
  .fhq-v8870-pack-art img{width:100%;height:100%;object-fit:contain;display:block;filter:drop-shadow(0 12px 18px rgba(0,0,0,.55))}\
...
injectStyle();
let tries=0;const t=setInterval(function(){tries++;if(install()||tries>50){clearInterval(t);setTimeout(function(){try{if(typeof fhqLoadShop==='function')fhqLoadShop()}catch(e){}},250)}},100);
console.log('[FootballHQ] V88.70 Overtime test-pack shop renderer active');
})();

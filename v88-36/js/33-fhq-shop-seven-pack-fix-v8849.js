/* ============================================================
   FOOTBALL HQ V88.49 — FRONTEND SHOP CATALOG FIX ONLY
   GitHub path: v88-36/js/33-fhq-shop-seven-pack-fix-v8849.js
   ============================================================ */
(function(){
'use strict';
...
const ART={
  rookie_cards:A+'pack-rookie-v8848.png?v=8849',
  scrimmage_pack:A+'pack-scrimmage-v8848.png?v=8849',
  gametime_pack:A+'pack-gametime-v8848.png?v=8849',
  redzone_pack:A+'pack-redzone-v8848.png?v=8849',
  fourthquarter_pack:A+'pack-fourthquarter-v8848.png?v=8849',
  hailmary_pack:A+'pack-hailmary-v8848.png?v=8849',
  primetime_pack:A+'pack-primetime-v8848.png?v=8849'
};

function esc2(s){return String(s==null?'':s).replace(/[&<>\"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]})}
function packPreview8849(pk){
  const url=ART[pk.id];
  if(url)return '<div class="fhq-v8849-pack-art"><img src="'+url+'" alt="'+esc2(pk.name)+'" draggable="false"></div>';
  if(typeof window.__fhq8848OldPackPreview==='function')return window.__fhq8848OldPackPreview(pk);
  if(typeof fhqPackPreview==='function')try{return fhqPackPreview(pk)}catch(e){}
  return '<div class="fhq-v8849-pack-art fallback"><strong>'+esc2(pk.name)+'</strong></div>';
}
...
function install(){
  ...
  const render=function(x){
    try{
      ...
      function packHTML(pk){
        return '<article class="fhq-shop-item fhq-pack-card '+esc2(pk.rarity||'')+'" data-shop-pack-card="'+esc2(pk.id)+'">'+
          '<div class="fhq-shop-preview">'+packPreview8849(pk)+'</div><h3>'+esc2(pk.name)+'</h3>'+ 
      ...
      let html='';
      if(filter==='all') html='<div class="fhq-v8831-shop-heading"><span>FEATURED</span><h2>Featured Packs</h2><p>Seven Football HQ card packs currently in rotation.</p></div>'+cardPacks.map(packHTML).join('');
    ...
    }catch(e){console.warn('[FHQ V88.49] render failed',e);return oldRender(x)}
  };
...
injectStyle();
let tries=0;const t=setInterval(function(){tries++;if(install()||tries>30){clearInterval(t);setTimeout(function(){try{if(typeof fhqLoadShop==='function')fhqLoadShop()}catch(e){}},200)}},100);
console.log('[FootballHQ] V88.49 seven-pack shop renderer active');
})();

/* ============================================================
   FOOTBALL HQ / TURF V88.84 — CURRENT SHOP CATALOG + NO FLASH

   The current pack renderer is unchanged. The only UX change is that
   the legacy shop grid stays invisible until this renderer has written
   the current pack catalog, preventing old packs from flashing first.
   ============================================================ */
(function(){
'use strict';
...
function install(){
  ...
  const render=function(x){
    try{
      ...
      g.style.visibility='hidden';g.style.opacity='0';
      ...
      revealCurrentGrid(g);
    }catch(e){console.warn('[FHQ V88.84] render failed',e);return oldRender(x)}
  };
...
function injectStyle(){
  ...
  #fhqShopGrid[data-turf-shop-ready="1"]{visibility:visible;opacity:1;transition:opacity .12s ease}\
...
console.log('[TURF] V88.84 shop no-flash renderer active');
})();
